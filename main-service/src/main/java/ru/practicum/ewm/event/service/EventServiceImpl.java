...
import ru.practicum.ewm.util.SearchSpecs;
...
public class EventServiceImpl implements EventService {
    ...
    private void checkEventDateTime(Event event) {
        if (!event.getEventDate().isAfter(LocalDateTime.now().plusHours(2))) {
            ...
            throw new DateTimeViolationException("eventDate", "Начало события не может быть в прошлом");
        }
    }
    ...
    private void checkParamDateTime(LocalDateTime start, LocalDateTime end) {
        if (start != null && end != null) {
            if (end.isBefore(start)) {
                throw new DateTimeViolationException("range", "Время окончания не может быть раньше даты начала");
            }
        }
    }
    ...
    private void updateEvent(Event event, UpdateEventUserRequest updateRequest, boolean isAdmin) {
        ...
        if (event.getState() == State.PUBLISHED) {
            throw new ConflictParameterException("state", "Событие уже опубликовано и не может быть изменено");
        }
